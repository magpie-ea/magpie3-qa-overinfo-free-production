...
Explore participants' behavior on the attention checks since aagain, an unexpectedly high proportion failed them. Again, it seems that participants just ignored the instructions and "correctly answered the trails".
```{r}
df_attention_fail %>%
  mutate(
...
df_clean_main_long <- df_clean_main %>%
  select(itemName, submission_id, targetOption, itemQuestion, competitor, sameCategory, otherCategory) %>%
   pivot_longer(cols = c(itemQuestion, competitor, sameCategory, otherCategory), names_to = 'answerType', values_to = "response") %>%
  mutate(
    categorized_response = case_when(
      targetOption == answerType ~ "target",
...
Explore target ratings in order to check if there are any unexpected results:
```{r}
...
Below, the responses are plotted by category described above.
...
df_clean_main_long %>% 
  ...
  # geom_path(aes(group = "submission_id"), alpha = 0.6) +
