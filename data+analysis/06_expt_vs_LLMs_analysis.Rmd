...
df_samples_human_global_summary %>% 
  ...
  ylab("Global proportion of answer type") +
  xlab("Answer type") +
  ggtitle("Response type proportions of human data vs\nneural model samples and probs")

```


```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.height=8, fig.width=6, eval=FALSE}
### Experiment 2 pilot

e2_pilot <- read_csv("data/results_QA-overinfo-contextDependent-freeTyping-pilot2_categorized.csv") %>% #read_csv("data/results_QA-overinfo-contextDependent-freeTyping-pilot1_categorized.csv") %>% filter(category != "yes") %>%
  mutate(category = factor(category, levels = c("taciturn", "competitor", "sameCategory", "mostSimilar", "otherCompetitor", "other"))) %>% filter(trial_type=="main")
e2_pilot %>% count(category)
e2_pilot_vignettes <- read_csv("../experiments/contextSensitive_free_production/trials/trials_split_cogsci_pilot1.csv") %>% 
  select(itemName, settingName, itemQuestion, competitor, sameCategory1, sameCategory2, otherCategory1, otherCategory2)
e2_pilot %>% count(itemName) %>% pull(n) %>% mean()
e2_pilot_wVignette <- left_join(e2_pilot, e2_pilot_vignettes, by = c("itemName", "settingName")) %>% filter(trial_type=="main")

e2_summary <- e2_pilot %>% group_by(itemName) %>%
  mutate(response_count = n())
e2_summary <- e2_summary %>% group_by(itemName, category) %>% 
...
e2_summary %>% 
  ggplot(aes(x = category, fill = category, y = responseCategory_proportion)) +
  geom_col() +
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.height=13, fig.width=4, eval=FALSE}
e2_pilot_wVignette_filtered <- e2_pilot_wVignette %>% filter((response_option != "yes") & (response_option != "no") & (response_option != "other")) %>%
...
e2_pilot_wVignette_filtered %>%
  ...
  facet_wrap(settingName ~., ncol= 1, scales = "free_y", 
             strip.position = "bottom", 
                labeller = as_labeller(c(blanket = "Transport | Sleepover", box = "BBQ | muffins", chair = "party | window", sugar = "baking | coffee"))) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
...
# same Cat, comp. most similar item, other cat
```
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.height=13, fig.width=4,eval=FALSE}
e2_pilot_wVignette_raw <- e2_pilot_wVignette %>% filter((response_option != "yes") & (response_option != "no") & (response_option != "other")) %>%
  separate_rows(response_option, sep=",") %>%
  mutate(response_option = gsub(" ", "", response_option)) %>%
  mutate(context_nr = case_when(
    itemName == 'sugar-coffee' ~ 'context1',
...
e2_pilot_wVignette_raw %>%
  ggplot(., aes(fill = category)) +
  geom_bar(data = e2_pilot_wVignette_raw %>% filter(context_nr == "context1"), aes(x = response_option)) +
  geom_bar(data = e2_pilot_wVignette_raw %>% filter(context_nr == "context2"), aes(x = response_option)) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  facet_wrap(settingName ~itemName, ncol= 2, scales = "free_x")
```
