...
df_clean_main_long_summary <- df_clean_main_long %>%
  ...
  tidyboot_mean(column = centered_response)
  #summarize(mean = mean(centered_response))

df_clean_main_long %>% 
  mutate(answerType = factor(answerType, 
                             levels = c("competitor", "sameCategory", "otherCategory"))) %>%
  ggplot(., aes(x = answerType, y = centered_response, color = answerType)) +
  geom_jitter(alpha = 0.7, width = 0.2) +
  geom_point(data = df_clean_main_long_summary, aes(x = answerType, y = mean), size = 3) +
...
df_clean_main_long_summary <- df_clean_main_long_summary %>% mutate(expt = "pilot 2")
df_clean_main_long <- df_clean_main_long %>% mutate(expt = "pilot 2")

df_bothPilots <- rbind(df_clean_main_long, df_pilot1)
